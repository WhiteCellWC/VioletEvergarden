...
const form = useForm({
    _method: 'PUT',
    name: props.envelopeType.data.name,
    description: props.envelopeType.data.description,
    price: props.envelopeType.data.price,
    stock: props.envelopeType.data.stock,
    discount: props.envelopeType.data.discount,
    images: [],
    delete_images: [],
    is_premium: props.envelopeType.data.is_premium,
    status: props.envelopeType.data.status
})

// Handlers
const submit = () => {
    form.post(route('envelope-types.update', { envelope_type: props.envelopeType.data.id }))
}
        ...
        <template #content>
            <div class="px-4 py-2 bg-gray-600">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-bold">Envelope Type</h2>
                </div>
            </div>

            <form @submit.prevent="submit" class="p-8 flex flex-col w-1/2 gap-6">
                <div class="flex flex-col gap-2">
                    <Label for="name" mandatory>Envelope Name</Label>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <ImageMultipleInput v-model="form.images" :uploadedImages="envelopeType.data.images"
